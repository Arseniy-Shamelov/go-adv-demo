...
func (handler *LinkHandler) Create() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		link := NewLink(body.Url) // создаем новую ссылку
		for {
			existedLink, _ := handler.LinkRepository.GetByHash(link.Hash) // поиск по хешу
			if existedLink == nil {
				break // если хеша не существует то выход
			}
			link.GenerateHash() // если сущетсвует то переделываем
		}
